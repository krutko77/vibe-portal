...
`es-trans_tests-results`, `es-trans-standard`) прописаны в registry.json на
их реальные домен/порт/каталог — для них nginx/certbot не трогаются, только
...
через `pm2 jlist`+`ss -ltnp` (порт слушателя, не название): `test-1` →
процесс `es-trans-test` (порт 3000), `test-2` → процесс `es-trans-standard`
(порт 3001, это и есть проект-папка `es-trans-standard`). Матчить по имени
папки без проверки портов — ошибочно, тексты `server.js` в обоих деплоях
оказались вообще идентичны (наивный `diff` тоже не отличил бы). Легаси-сайт
`es-trans-test`/`test-1` (папка `es-trans-test-OM`) по-прежнему НЕ занесён —
не запрашивалось, сверить порт/процесс/папку так же вручную перед добавлением.
